...
public class CompletableFutureTest05 {
    ...
    private static void searchPricesWithDiscountAsync(StoreServiceWithDiscount service){
        ...
        var completableFutures = stores.stream()
                ...
                .map(cf -> cf.thenCompose(quote -> CompletableFuture
                        .supplyAsync(() -> service.applyDiscount(quote))))
                .map(cf -> cf.thenAccept(store ->
                        System.out.printf("%s finished in %d%n", store, (System.currentTimeMillis() - start))))
                .toArray(CompletableFuture[]::new);

//        final CompletableFuture<Void> voidCompletableFuture = CompletableFuture.allOf(completableFutures);
        final CompletableFuture<Object> voidCompletableFuture = CompletableFuture.anyOf(completableFutures);
